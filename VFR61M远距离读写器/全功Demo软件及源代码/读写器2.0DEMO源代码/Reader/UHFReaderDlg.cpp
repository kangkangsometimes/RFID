...
BEGIN_MESSAGE_MAP(CUHFReaderDlg, CDialog)
	...
	ON_MESSAGE(UM_SCAN_DBCLK, &CUHFReaderDlg::FillIPAndPort)
	//}}AFX_MSG_MAP
	ON_WM_DESTROY()
END_MESSAGE_MAP()
...
LRESULT CUHFReaderDlg::FillIPAndPort(WPARAM wParam, LPARAM lParam)
{
	LPIPANDTCPPORT info = (LPIPANDTCPPORT)wParam;
	CString sIP = info->sIP;
	CString sPort = info->sTcpPort;

	SetDlgItemText(IDC_IP_ADDR_IPADDRESS, sIP);
	SetDlgItemText(IDC_IP_PORT_EDIT, sPort);
...
{
	FILE *pfile = fopen("./Language.dat", "r");
	fscanf(pfile, "%d", &LanguageFlag);
	fclose(pfile);
	return 0;
}
...
{	
	...
	if(0 == Language)
	{
		...
		m_system_tab.InsertItem(1,"16通道天线配置");
		m_system_tab.InsertItem(2,"ISO18000-6B");
		m_system_tab.InsertItem(3,"EPC GEN2");
		m_system_tab.InsertItem(4, _T("扫描"));
	}
	else
	{
		m_system_tab.InsertItem(0,"System Config");
		m_system_tab.InsertItem(1,"16 Ant Config");
		m_system_tab.InsertItem(2,"ISO18000-6B");
		m_system_tab.InsertItem(3,"EPC GEN2");
		m_system_tab.InsertItem(4, _T("Scan"));
	}
	...
	m_ant.Create(IDD_ANTCONFIG_DIALOG, GetDlgItem(IDC_SYSTEM_TAB));
	...
	m_ScanDlg.Create(IDD_DLG_SCAN, GetDlgItem(IDC_SYSTEM_TAB));
