...
void CSystemConfig::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_MAC_EDIT, m_mac);
	DDX_Text(pDX, IDC_SNO_EDIT, m_sno);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CSystemConfig, CDialog)
	...
	ON_BN_CLICKED(IDC_SETSNOMAC_BUTTON, OnSetSnoMacButton)
	ON_BN_CLICKED(IDC_QUERYSNOMAC_BUTTON, OnQuerySnoMacButton)
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_BTN_AASET, &CSystemConfig::OnBnClickedBtnAaset)
	ON_BN_CLICKED(IDC_BTN_SETLB, &CSystemConfig::OnBnClickedBtnSetlb)
	ON_BN_CLICKED(IDC_BTN_GETLB, &CSystemConfig::OnBnClickedBtnGetlb)
	ON_BN_CLICKED(IDC_RADIO_TYPE6B, &CSystemConfig::OnBnClickedRadioType6b)
	ON_BN_CLICKED(IDC_RADIO_TYPE6C, &CSystemConfig::OnBnClickedRadioType6c)
	ON_BN_CLICKED(IDC_BTN_GOBACK, &CSystemConfig::OnBnClickedBtnGoback)
END_MESSAGE_MAP()
...
void CSystemConfig::OnQueryInterfaceButton() 
{
	...
	// 自动读卡时间间隔 2015-11-11
	QueryAutoInterval();
	// 自动、触发模式下标签类型 2015-11-11
	QueryAutoTriggerTagType();
	// 自动模式下区域配置
	QueryAutoAreaConfig();
}
...
void CSystemConfig::GetSingleFastTagMode()//query reader single fast tag mode
{
	...
	if(0x00 == Config_GetSingleFastTagMode(CS, Mode, 0xFF))
	{
		...
		if(0x00 == Mode)
		{
			...
			((CButton *)GetDlgItem(IDC_MORETAG_RADIO))->SetCheck(0);
			if(0 == Language)
				m_tagmode = "单卡快读模式";
			else
				m_tagmode = "single tag fast mode";
		...
		else
		{
			...
			((CButton *)GetDlgItem(IDC_LITERTAG_RADIO))->SetCheck(0);
			if(0 == Language)
				m_tagmode = "多卡模式";
			else
				m_tagmode = "multitag mode";
...
void CSystemConfig::SetInternetAccess()//set reader internet port parameter
{
	...
	InternetPort = 100;//atoi(m_internetport);
...
int CSystemConfig::TriggerTime(int ReaderNum, CString Text)
{
	...
	if( !triggertime && Text == "设置" || Text == "Set")
	{
		TempStr = "触发时间不能为空或者0!";
...
void CSystemConfig::LanguageChinese()
{
	...
	((CButton *)GetDlgItem(IDC_STYLE_STATIC))->SetWindowText("韦根类型");

	((CButton *)GetDlgItem(IDC_STATIC_SNO))->SetWindowText("序列号:");
	((CButton *)GetDlgItem(IDC_STATIC_MAC))->SetWindowText("MAC地址");
	((CButton *)GetDlgItem(IDC_SETSNOMAC_BUTTON))->SetWindowText("设置");
	((CButton *)GetDlgItem(IDC_QUERYSNOMAC_BUTTON))->SetWindowText("查询");
}
...
void CSystemConfig::LanguageEnglish()
{
	...
	((CButton *)GetDlgItem(IDC_STYLE_STATIC))->SetWindowText("WeigenType");

	((CButton *)GetDlgItem(IDC_STATIC_SNO))->SetWindowText("SerialNo:");
	((CButton *)GetDlgItem(IDC_STATIC_MAC))->SetWindowText("MAC Addr");
	((CButton *)GetDlgItem(IDC_SETSNOMAC_BUTTON))->SetWindowText("Set");
	((CButton *)GetDlgItem(IDC_QUERYSNOMAC_BUTTON))->SetWindowText("Query");
}
...
void CSystemConfig::OnSetSnoMacButton() 
{
	// TODO: Add your control notification handler code here
	if(((CButton *)GetDlgItem(IDC_CHECK_SNO))->GetCheck())
		SetSerialNo(255);
	if(((CButton *)GetDlgItem(IDC_CHECK_MAC))->GetCheck())
		SetMACAddr(255);
}

void CSystemConfig::OnQuerySnoMacButton() 
{
	// TODO: Add your control notification handler code here
	if(((CButton *)GetDlgItem(IDC_CHECK_SNO))->GetCheck())
		QuerySerialNo(255);
...
int CSystemConfig::SetSerialNo(int ReaderNum)
{
	UpdateData(true);
	CString TempStr = "";
	CString TempStrEnglish = "";
	CString Sno = "";
	Sno = m_sno;
	if(Sno.GetLength() <= 0 || Sno.GetLength() > 20)
	{
		TempStr = "设置读写器序列号长度错误!(1~20)";
		TempStrEnglish = "Reader serial No set length error!(1~10)";
		if(0 == Language)
			CUHFReaderDlg::ListBoxAdd(TempStr);
		else
			CUHFReaderDlg::ListBoxAdd(TempStrEnglish);
	...
	int PLen = Sno.GetLength()/2;
	unsigned char PSetData[10] = {0};
	
	CUHFReaderDlg::HexStrToByte(PSetData, Sno);
	if(0x00 == Parameter_SetReader(CS, PAddr, PLen, PSetData, ReaderAddr))
	{
		TempStr = "设置读写器序列号成功!";
		TempStrEnglish = "Reader serial No set success!";
	}
	else
	{
		TempStr = "设置读写器序列号失败!";
		TempStrEnglish = "Reader serial No set fail!";
	}
	if(0 == Language)
		CUHFReaderDlg::ListBoxAdd(TempStr);
	else
...
int CSystemConfig::QuerySerialNo(int ReaderNum)
{
	UpdateData(true);
	CString TempStr = "";
	CString TempStrEnglish = "";
	...
	if(0x00 == Parameter_GetReader(CS, PAddr, PLen, PRefData, ReaderAddr))
	{
		Sno = CUHFReaderDlg::ByteToHexStr(PRefData, PLen);
		m_sno = Sno;
		TempStr = "查询读写器序列号成功!";
...
int CSystemConfig::SetMACAddr(int ReaderNum)
{
	UpdateData(true);
	CString TempStr = "";
	CString TempStrEnglish = "";
	...
	{
		TempStr = "设置读写器MAC地址长度错误!(6)";
		TempStrEnglish = "Reader MacAddr set length error!(6)";
		if(0 == Language)
			CUHFReaderDlg::ListBoxAdd(TempStr);
		else
			CUHFReaderDlg::ListBoxAdd(TempStrEnglish);
	...
	unsigned char ReaderAddr = (unsigned char)(ReaderNum & 0xFF);
	unsigned char PAddr = 0xB0;
	int PLen = 6;
	unsigned char PSetData[6] = {0};
	CUHFReaderDlg::HexStrToByte(PSetData, MacAddr);

	if(0x00 == Parameter_SetReader(CS, PAddr, PLen, PSetData, ReaderAddr))
	{
		TempStr = "设置读写器MAC地址成功!";
		TempStrEnglish = "Reader MacAddr set success!";
	}
	else
	{
		TempStr = "设置读写器MAC地址失败!";
		TempStrEnglish = "Reader MacAddr set fail!";
	}
	if(0 == Language)
		CUHFReaderDlg::ListBoxAdd(TempStr);
	else
...
int CSystemConfig::QueryMACAddr(int ReaderNum)
{
	UpdateData(true);
	CString TempStr = "";
	CString TempStrEnglish = "";
	...
	if(0x00 == Parameter_GetReader(CS, PAddr, PLen, PRefData, ReaderAddr))
	{
		MacAddr = CUHFReaderDlg::ByteToHexStr(PRefData, PLen);
		m_mac = MacAddr;
		TempStr = "查询读写器MAC地址成功!";
		TempStrEnglish = "Reader MacAddr query success!";
	}
	else
	{
		TempStr = "查询读写器MAC地址失败!";
		TempStrEnglish = "Reader MacAddr query fail!";
	}
	if(0 == Language)
		CUHFReaderDlg::ListBoxAdd(TempStr);
	else
...
void CSystemConfig::OnBnClickedBtnAaset()
{
	// TODO: Add your control notification handler code here
	unsigned char mode = -1;
	if ( ( (CButton *)GetDlgItem(IDC_RADIO_AAOPEN) )->GetCheck() )
	{
		mode = 0;
	}
	if ( ( (CButton *)GetDlgItem(IDC_RADIO_AACLOSE) )->GetCheck() )
	{
		mode = 1;
	}
	if ( ( (CButton *)GetDlgItem(IDC_RADIO_AA) )->GetCheck() )
	{
		mode = 2;
	}
	...
	int iRet = Config_SetTestMode(CS, mode, 0xFF);

	if (iRet == 0)
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置测试模式成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置测试模式失败"));
	}
}

void CSystemConfig::OnBnClickedBtnSetlb()
{
	// TODO: Add your control notification handler code here
	unsigned char state = 0;

	if ( ( (CButton *)GetDlgItem(IDC_CHECK_LED) )->GetCheck())
	{
		state += 1;
	}

	if ( ( (CButton *)GetDlgItem(IDC_CHECK_BUZZER) )->GetCheck())
	{
		state += 2;
	}

	int iRet = Parameter_SetReader(CS, 0x1b, 1, &state, 0xFF);
	if (0 == iRet)
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置Led和蜂鸣器成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置Led和蜂鸣器失败"));
	}
}

void CSystemConfig::OnBnClickedBtnGetlb()
{
	// TODO: Add your control notification handler code here
	unsigned char state = 0;
	int iRet = Parameter_GetReader(CS, 0x1b, 1, &state, 0xFF);
	
	( (CButton *)GetDlgItem(IDC_CHECK_LED) )->SetCheck(0);
	( (CButton *)GetDlgItem(IDC_CHECK_BUZZER) )->SetCheck(0);
	if (0 == iRet)
	{
		CUHFReaderDlg::ListBoxAdd(_T("获取Led和蜂鸣器成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("获取Led和蜂鸣器失败"));
		return;
	}

	if (state & 1)
	{
		( (CButton *)GetDlgItem(IDC_CHECK_LED) )->SetCheck(1);
		
	}
	if (state & 2)
	{
		( (CButton *)GetDlgItem(IDC_CHECK_BUZZER) )->SetCheck(1);
	}
	
}

int CSystemConfig::QueryAutoInterval(void)
{
	unsigned char interval = 0;
	int iRet = Parameter_GetReader(CS, 0x91, 1, &interval, 0xFF);
	if (iRet == 0)
	{
		int msinterval = interval * 10;
		SetDlgItemInt(IDC_EDIT_AUTOINTERVAL, msinterval);
		CUHFReaderDlg::ListBoxAdd(_T("获取自动模式时间间隔成功"));
		return 0;
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("获取自动模式时间间隔失败"));
		return iRet;	
	}
...
int CSystemConfig::SetAutoInterval(void)
{
	CString sInterval(_T(""));
	GetDlgItemText(IDC_EDIT_AUTOINTERVAL, sInterval);
	if (sInterval.Trim().IsEmpty())
	{
		return -1;
	}
	
	int msinterval = _ttoi((LPCTSTR)sInterval);
	unsigned char interval = msinterval / 10;
	int iRet = Parameter_SetReader(CS, 0x91, 1, &interval, 0xFF);
	if (iRet == 0)
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置自动模式时间间隔成功"));
		return 0;
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置自动模式时间间隔失败"));
		return iRet;	
	}
}

int CSystemConfig::QueryAutoTriggerTagType(void)
{
	unsigned char TagType = 0;
	int iRet = Parameter_GetReader(CS, 0x82, 1, &TagType, 0xFF);
	if (iRet == 0)
	{
		if (TagType == 0)
		{
			( (CButton *)GetDlgItem(IDC_RADIO_TYPE6C) )->SetCheck(1);
			( (CButton *)GetDlgItem(IDC_RADIO_TYPE6B) )->SetCheck(0);
		}
		else if (TagType == 1)
		{
			( (CButton *)GetDlgItem(IDC_RADIO_TYPE6C) )->SetCheck(0);
			( (CButton *)GetDlgItem(IDC_RADIO_TYPE6B) )->SetCheck(1);
		}
		else
		{
			( (CButton *)GetDlgItem(IDC_RADIO_TYPE6C) )->SetCheck(0);
		...
		CUHFReaderDlg::ListBoxAdd(_T("获取标签类型成功"));
		return 0;
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("获取标签类型失败"));
		return iRet;	
	}
}

int CSystemConfig::SetAutoTriggerTagType(void)
{

	unsigned char TagType = -1;
	if (( (CButton *)GetDlgItem(IDC_RADIO_TYPE6C) )->GetCheck())
	{
		TagType = 0;
	}
	if (( (CButton *)GetDlgItem(IDC_RADIO_TYPE6B) )->GetCheck())
	{
		TagType = 1;
	}
	...
	int iRet = Parameter_SetReader(CS, 0x82, 1, &TagType, 0xFF);
	if (iRet == 0)
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置自动模式时间间隔成功"));
		return 0;
...
int CSystemConfig::QueryAutoAreaConfig(void)
{
	unsigned char area;
	unsigned char out_style;
	unsigned char head_addr;
	unsigned char len;
	unsigned char isfilter;
	unsigned char filter_window;

	int iRet;

	iRet = Parameter_GetReader(CS, 0x83, 1, &area, 255);
	if (iRet == 0)
	{
		if (area >= 0)
		{
			((CComboBox *)GetDlgItem(IDC_COMBO_AREA))->SetCurSel(area);
		}
		CUHFReaderDlg::ListBoxAdd(_T("读取读标签区域成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("读取读标签区域失败"));
	}


	iRet = Parameter_GetReader(CS, 0x95, 1, &out_style, 255);
	if (iRet == 0)
	{
		if (out_style >= 0)
		{
			((CComboBox *)GetDlgItem(IDC_COMBO_DATAOUTTYPE))->SetCurSel(out_style);
		}
		CUHFReaderDlg::ListBoxAdd(_T("读取数据输出方式成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("读取数据输出方式失败"));
	}

	iRet = Parameter_GetReader(CS, 0x84, 1, &head_addr, 255);
	if (iRet == 0)
	{

		SetDlgItemInt(IDC_EDIT_HEADADDR, head_addr);
		

		CUHFReaderDlg::ListBoxAdd(_T("读取首地址成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("读取首地址失败"));
	}


	iRet = Parameter_GetReader(CS, 0x85, 1, &len, 255);
	if (iRet == 0)
	{

		SetDlgItemInt(IDC_EDIT_LEN, len);
		

		CUHFReaderDlg::ListBoxAdd(_T("读取长度成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("读取长度失败"));
	}


	iRet = Parameter_GetReader(CS, 0x93, 1, &isfilter, 255);
	if (iRet == 0)
	{
		((CButton *)GetDlgItem(IDC_CHECK_FILTER))->SetCheck(isfilter);

		CUHFReaderDlg::ListBoxAdd(_T("读取过滤窗口成功"));
	}
	else
	{
		isfilter = 0;
		((CButton *)GetDlgItem(IDC_CHECK_FILTER))->SetCheck(isfilter);
		CUHFReaderDlg::ListBoxAdd(_T("读取过滤窗口失败"));
	}
	...
	iRet = Parameter_GetReader(CS, 0x94, 1, &filter_window, 255);
	if (iRet == 0)
	{
		SetDlgItemInt(IDC_EDIT_FILTERWINDOW, filter_window);
		CUHFReaderDlg::ListBoxAdd(_T("读取过滤窗口时间成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("读取过滤窗口时间失败"));
	}
...
int CSystemConfig::SetAutoAreaConfig(void)
{
	unsigned char area;
	unsigned char out_style;
	unsigned char head_addr;
	...
	area = ((CComboBox *)GetDlgItem(IDC_COMBO_AREA))->GetCurSel();
	out_style = ((CComboBox *)GetDlgItem(IDC_COMBO_DATAOUTTYPE))->GetCurSel();
	head_addr = GetDlgItemInt(IDC_EDIT_HEADADDR);
	len = GetDlgItemInt(IDC_EDIT_LEN);
	isfilter = ((CButton *)GetDlgItem(IDC_CHECK_FILTER))->GetCheck();
	if (isfilter)
	{
		filter_window = GetDlgItemInt(IDC_EDIT_FILTERWINDOW);
	}

	int iRet;

	iRet = Parameter_SetReader(CS, 0x83, 1, &area, 255);
	if (iRet == 0)
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置读标签区域成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置读标签区域失败"));
	}


	iRet = Parameter_SetReader(CS, 0x95, 1, &out_style, 255);
	if (iRet == 0)
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置数据输出方式成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置数据输出方式失败"));
	}

	iRet = Parameter_SetReader(CS, 0x84, 1, &head_addr, 255);
	if (iRet == 0)
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置首地址成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置首地址失败"));
	}


	iRet = Parameter_SetReader(CS, 0x85, 1, &len, 255);
	if (iRet == 0)
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置长度成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置长度失败"));
	}


	iRet = Parameter_SetReader(CS, 0x93, 1, &isfilter, 255);
	if (iRet == 0)
	{

		CUHFReaderDlg::ListBoxAdd(_T("设置过滤窗口成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置过滤窗口失败"));
	}
	...
	iRet = Parameter_SetReader(CS, 0x94, 1, &filter_window, 255);
	if (iRet == 0)
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置过滤窗口时间成功"));
	}
	else
	{
		CUHFReaderDlg::ListBoxAdd(_T("设置过滤窗口时间失败"));
	}
...
void CSystemConfig::OnBnClickedRadioType6b()
{
	// TODO: Add your control notification handler code here
	GetDlgItem(IDC_COMBO_AREA)->EnableWindow(FALSE);
}

void CSystemConfig::OnBnClickedRadioType6c()
{
	// TODO: Add your control notification handler code here
...
void CSystemConfig::OnBnClickedBtnGoback()
{
	// TODO: Add your control notification handler code here
	((CButton *)GetDlgItem(IDC_ANT1_CHECK))->SetCheck(1);
	((CButton *)GetDlgItem(IDC_ANT2_CHECK))->SetCheck(0);
	...
	SetDlgItemInt(IDC_ANTENNA1_EDIT, 30);
	SetDlgItemInt(IDC_ANTENNA2_EDIT, 0);
	SetDlgItemInt(IDC_ANTENNA3_EDIT, 0);
	SetDlgItemInt(IDC_ANTENNA4_EDIT, 0);

	((CButton *)GetDlgItem(IDC_RADIO_AAOPEN))->SetCheck(0);
	((CButton *)GetDlgItem(IDC_RADIO_AACLOSE))->SetCheck(0);
	((CButton *)GetDlgItem(IDC_RADIO_AA))->SetCheck(0);

	SetDlgItemText(IDC_IPADDRESS, _T("192.168.1.200"));
	SetDlgItemInt(IDC_INTERNETPORT_EDIT, 100);
	SetDlgItemText(IDC_GATEWAY, _T("192.168.1.1"));
	SetDlgItemText(IDC_MASKCODE, _T("255.255.255.0"));
	...
	((CButton *)GetDlgItem(IDC_IOPORT1_RADIO))->SetCheck(1);
	((CButton *)GetDlgItem(IDC_IOPORT2_RADIO))->SetCheck(0);
	((CButton *)GetDlgItem(IDC_RELAY_RADIO))->SetCheck(0);
	...
	((CComboBox *)GetDlgItem(IDC_COMBO_AREA))->SetCurSel(1);
	((CComboBox *)GetDlgItem(IDC_COMBO_DATAOUTTYPE))->SetCurSel(0);

	SetDlgItemInt(IDC_EDIT_HEADADDR, 4);
	SetDlgItemInt(IDC_EDIT_LEN, 6);

	((CButton *)GetDlgItem(IDC_CHECK_FILTER))->SetCheck(1);
	SetDlgItemInt(IDC_EDIT_FILTERWINDOW, 1);
	...
	((CButton *)GetDlgItem(IDC_WEIGEN_CHECK))->SetCheck(1);
	((CButton *)GetDlgItem(IDC_RELAY_CHECK))->SetCheck(0);
	((CComboBox *)GetDlgItem(IDC_WEIGENSTYLE_COMBO))->SetCurSel(0);
	SetDlgItemInt(IDC_EDIT_AUTOINTERVAL, 100);
	
	
	((CButton *)GetDlgItem(IDC_CHECK_MAC))->SetCheck(1);
	((CButton *)GetDlgItem(IDC_CHECK_SNO))->SetCheck(1);
